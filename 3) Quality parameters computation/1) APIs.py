# FIXING THE VISUALIZATION OF THE BARCODE STRUCTURE
...
############################ OVERALL FUNCTION 
def detect_and_refine_boundingBox(image_path, use_same_threshold=False, compute_barcode_structure_algorithm=1, verbose_timing=False,
    ...
    visualization_dict : dict, optional
        ...
        The keys are: 'visualize_original_image', 'visualize_original_image_boundingBox', 'visualize_rotated_image_boundingBox',
    ...
    if visualization_dict=='all':
        ...
        visualization_dict = {}
        visualization_dict['visualize_original_image'] = True
...
def sort_bb_points_for_visualization(bb_points_sorted):
    """Function which sorts the bounding box points differently, for making the bb compliant with the visualization API"""
...
def compute_barcode_structure(roi_image, bb_width, bb_height, algorithm=1, threshold=None, verbose=False, visualize_refined_bb=False,
    ...
    barcode_structure_dict : dict
        ...
        barcode_structure_dict['bars_start'] : list of int
            List containing the starting pixel of each bar
        barcode_structure_dict['bars_width'] : list of int
            List containing the width of each bar
        barcode_structure_dict['bars_halfHeightUp'] : list of int
            List containing the half heigth up of each bar
        barcode_structure_dict['bars_halfHeightDown'] : list of int
    ...
    if visualize_barcode_structure:
        ...
        n_bars = len(bars_start)
        for b in range(n_bars):
            roi_image_show[[half_height-bars_halfHeightUp[b]-1,half_height+bars_halfHeightDown[b]-1],bars_start[b]:bars_start[b]+bars_width[b],:] = np.array([255,0,0])
    ...
    barcode_structure_dict = {}
    barcode_structure_dict['bars_start'] =  bars_start 
    barcode_structure_dict['bars_width'] =  bars_width   
    barcode_structure_dict['bars_halfHeightUp'] =  bars_halfHeightUp   
    barcode_structure_dict['bars_halfHeightDown'] =  bars_halfHeightDown     
    
    return barcode_structure_dict
...
def find_and_fix_wrong_bar(roi_image, bb_height, barcode_structure_dict, level=0.02, visualize_fixed_barcode_structure=False):
    """Find if, in the given barcode structure, a detected bar is wrong, which means that it is not an actual bar. If a wrong 
    bar is present, it is fixed, which means that it is removed from the current barcode structure.

    A bar is wrong if it is an outlier with respect to the others. 

    If a wrong bar is found, the given input dictionary is modified.
    ...
    level : float, optional
        Level to use for detecting the outlier bar, by default 0.02
...
def refine_roi(gray_rot, bb_points_sorted_rot, bb_height, bb_width, barcode_structure_dict, 
                visualize_refined_roi_withQuantities=False, visualize_refined_roi=False): 
    """Refine the ROI image containing the barcode.

    This refinement follows the following criteria:
    - the height of the ROI image is exactly the minimum height of a barcode bar;
    - the width of the ROI image contains exactly the barcode bars, plus 10X before the first bar and 10X after the last bar,
      where X is the minimum width of a bar.

    Parameters
    ----------
    gray_rot : np.array
        Original gray image, rotated in order to have the barcode bars perfectly vertical.
        (At most two rotations have been performed: rotation for making the barcode perfectly aligned with the image axes and
         rotation for fixing the horizontal bars problem, if present.)
    bb_points_sorted_rot : np.array
        Array containing the coordinates of the four verteces of the bounding box enclosing the barcode.
        These bounding box points are sorted according to the ordering top-left -> top-rigth -> bottom-left -> bottom-right.
        This bounding box is the one rotated in order to have the barcode bars perfectly vertical.
        (At most two rotations, as explained for `gray_rot`)
    bb_height : int
        Height of the bounding box.
    bb_width : int
        Width of the bounding box.
    barcode_structure_dict : dict
        Dictionary containing the barcode structure. 
        In particular, we are interested in the following quantities.
        barcode_structure_dict['first_bar_x'] : int
            Horizontal coordinate of the first pixel of the first bar in the barcode.
        barcode_structure_dict['last_bar_x'] : int
            Horizontal coordinate of the last pixel of the last bar in the barcode.
        barcode_structure_dict['X'] : int
            Minimum width of a barcode bar.
        barcode_structure_dict['min_half_height_up'] : int
            Minimum half height up of a barcode bar.
        barcode_structure_dict['min_half_height_down'] : int
            Minimum half height down of a barcode bar.
    visualize_refined_roi_withQuantities : bool, optional
        Whether to visualize or not the refined ROI image with the computed quantities, by default False
    visualize_refined_roi : bool, optional
        Whether to visualize or not the refined ROI image, by default False

    Returns
    ----------
    roi_image_ref : np.array
        Rotated image cropped around the refined ROI
    bb_points_sorted_rot_ref : np.array
        Array containing the coordinates of the four verteces of the bounding box enclosing the barcode.
        These bounding box points are sorted according to the ordering top-left -> top-rigth -> bottom-left -> bottom-right.
        This bounding box is the one rotated and refined: final bounding box.
    bb_width_ref : int
        Width of the refined roi image.
    bb_height_ref : int
        Height of the refined roi image. Basically, it is the minimum height of a barcode bar.
    ...
    roi_image_ref = gray_rot[int(bb_points_sorted_rot_ref[0][1]):int(bb_points_sorted_rot_ref[2][1])+1, 
                                int(bb_points_sorted_rot_ref[0][0]):int(bb_points_sorted_rot_ref[1][0])+1].copy()
    bb_height_ref, bb_width_ref  = roi_image_ref.shape
