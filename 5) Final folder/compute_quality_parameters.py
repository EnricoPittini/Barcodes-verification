...
def compute_quality_parameters(roi_image_ref, n_scanlines=10, visualize_scanlines_onRoiImage=False, 
    ...
    for i, scanline_index in enumerate(scanlines_indices):
        ...
        if visualize_scanlines_qualityParameters:  # Visualize the scan reflectance profile and the quality parameters of the
                                                   # currrent scanline
...
#################### FUNCTIONS FOR COMPUTING THE GRADES/VALUES OF THE QUALITY PARAMETERS    
...
################## FUNCTIONS FOR COMPUTING THE QUALITY PARAMETERS ON ONE SCANLINE  (PRIVATE FUNCTIONS)
...
def _compute_quality_parameters_scanline(scanline):
    ...
    ----------
    scanline : np.array
        Mono-dimensional array, represnting the scanline. Basically, it is a 'row' of the refined ROI image: it contains pixels 
        intensities.
